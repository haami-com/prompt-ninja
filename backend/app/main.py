...
from .models import Brief, BriefEnhancementResult, CouncilResult, GeneratedPromptTestRequest, GeneratedPromptTestResult, PromptExportRequest
...
async def _extract_numbered_files(files: list[UploadFile]) -> list[dict[str, str | int]]:
    numbered_sources = []
...
async def enhance_brief(
    ...
    if len(request_text.strip()) < 8:
        raise HTTPException(status_code=422, detail="Describe what you need in at least 8 characters.")
    if len(request_text) > 12000:
        raise HTTPException(status_code=422, detail="Keep the request under 12,000 characters.")
    if len(files) > 5:
        raise HTTPException(status_code=413, detail="Upload up to five reference files.")
    if not os.getenv("OPENAI_API_KEY"):
        raise HTTPException(status_code=503, detail="Set OPENAI_API_KEY to enhance the brief.")
    numbered_sources = await _extract_numbered_files(files)
    try:
        return await BriefEnhancer().enhance(request_text, numbered_sources)
    except Exception as exc:
        raise HTTPException(status_code=502, detail="Brief enhancement failed: %s" % exc) from exc
...
async def generate(
    ...
    async def events():
        try:
            async for item in PromptCouncil(creator_models=selected_creators, judge_model=judge_model or None, creator_prompts=selected_creator_prompts or None, judge_prompt=judge_prompt or None).stream(brief):
