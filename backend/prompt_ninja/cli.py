"""Click commands for generating, validating, testing, and updating prompts."""
...
from .models import AgentMessage, Brief, CouncilResult, PromptExportRequest
...
from .prompt_export import (
    prompt_filename,
    prompt_from_export_request,
    write_generated_output_model,
)
from .core import (
...
def _write_generated_prompt(path: Path, goal: str, result: CouncilResult) -> None:
    definition = deepcopy(result.prompt_definition) or None
    if definition is not None and result.output_model:
        try:
            output_path = write_generated_output_model(path, result.output_model)
        except ValueError as exc:
            raise click.ClickException(str(exc)) from exc
        definition["metadata"]["output"] = output_path
...
async def _generate(brief: Brief) -> CouncilResult:
    ...
    async for item in PromptCouncil().stream(brief):
        if isinstance(item, CouncilResult):
            result = item
            click.echo("OK Board complete - prompt compiled and tested")
        elif isinstance(item, AgentMessage):
            if item.status == "started":
                click.echo("-> %s" % item.title)
            elif item.status == "complete":
                detail = " - %s" % item.summary if item.summary else ""
                click.echo("OK %s%s" % (item.title, detail))
            else:
                detail = " - %s" % item.summary if item.summary else ""
                click.echo("ERROR %s%s" % (item.title, detail), err=True)
    if result is None:
...
async def _run_tests(
    ...
    if on_start is None and on_result is None:
        return await run_prompt_artifact_tests(
            prompt, judge_model, prompt_client=client
        )
    if not prompt.tests:
...
def _show_verbose_result(
    ...
    if result.prompt_suggestion:
        sections.append("suggested prompt change:\n%s" % result.prompt_suggestion)
    if result.test_suggestion:
        sections.append("suggested test change:\n%s" % result.test_suggestion)
    output = "\n".join(sections)
    if console is None:
        click.echo(output)
    else:
...
def _show_report(
    ...
    if console is None:
        ...
        for result in report.results:
            ...
            if result.passed:
                ...
                click.echo(
                    "%s %s / %s (%s)"
                    % (state, report.prompt_name, result.name, score)
                )
    ...
    for result in report.results:
        ...
        table.add_row(
            ...
            Text(
                _short_text(detail) if result.passed else "See failure details below.",
...
async def _rewrite_prompt_file(
    prompt_file: Path,
    rewrite_prompt: PromptNinja,
    variables: dict[str, str],
    model: str,
) -> str:
    original = prompt_file.read_text(encoding="utf-8")
    updated = await rewrite_prompt.run_openrouter(
        {"prompt_toml": original, **variables}, model=model
    )
...
async def _repair_prompt_file(
    prompt_file: Path, feedback: str, model: str
) -> tuple[str | None, PromptTestReport]:
    original = prompt_file.read_text(encoding="utf-8")
    prompt = PromptNinja.from_file(prompt_file)
    try:
        updated = await update_prompt_artifact(prompt, feedback, model)
    except PromptNinjaError as exc:
        raise click.ClickException(str(exc)) from exc
    report = await run_prompt_artifact_tests(updated, model)
    if not report.passed:
        return None, report
    backup = prompt_file.with_suffix(prompt_file.suffix + ".bak")
    backup.write_text(original, encoding="utf-8")
    prompt_file.write_text(updated.to_toml(), encoding="utf-8")
    return str(backup), report


async def _repair_invalid_prompt_file(
    prompt_file: Path, validation_error: str, model: str
) -> str:
    return await _rewrite_prompt_file(
        prompt_file,
        PROMPTS.prompt_repair,
...
def update(prompt_file: Path, feedback: str, model: str) -> None:
    """Update a prompt implementation, then verify its preserved tests."""
    ...
    backup, report = asyncio.run(_repair_prompt_file(prompt_file, feedback, model))
    _show_report(report, verbose=True, console=None)
    _show_test_summary([report], console=None)
    if backup is None:
        raise click.ClickException(
            "Candidate prompt failed its preserved semantic test contract; "
            "the original artifact was not changed."
        )
    ...
    for prompt_file in paths:
        try:
            ...
            if fix:
                try:
                    backup = asyncio.run(
                        _repair_invalid_prompt_file(prompt_file, str(exc), model)
                    )
                    PromptNinja.from_file(prompt_file)
                    failures -= 1
                    click.echo(
