...
const modelLabel = (model) => String(model || "Unknown model").replace(/^gpt-/, "GPT-").replace(/-([a-z])/g, (_, letter) => ` ${letter}`);
              ...
              {result && <Box mt={7} pt={7} borderTop="1px solid" borderColor="blackAlpha.100">
                ...
                <Box mt={5} pt={5} borderTop="1px solid" borderColor="blackAlpha.100">
                  ...
                  <SimpleGrid columns={{ base: 1, md: 2 }} gap={3} mt={4}>
                    <FormControl><FormLabel fontSize="xs" color="gray.500">Runner model</FormLabel><Select value={testModel} onChange={(event) => setTestModel(event.target.value)} size="sm" bg="gray.50">{modelOptions.map((value) => <option key={value} value={value}>{modelLabel(value)}</option>)}</Select></FormControl>
                    <FormControl><FormLabel fontSize="xs" color="gray.500">Judge model</FormLabel><Select value={models.judge} onChange={(event) => setModels((current) => ({ ...current, judge: event.target.value }))} size="sm" bg="gray.50">{modelOptions.map((value) => <option key={value} value={value}>{modelLabel(value)}</option>)}</Select></FormControl>
                  </SimpleGrid>
